...
if($_GET['action'] == "modify")
{
	...
	if (!!$_rows = _fetch_array("SELECT uniqid FROM tb_user WHERE username='{$_COOKIE['username']}' LIMIT 1"))
	{
		...
		_close();
		_session_destroy();
		_location('恭喜你，修改成功！','member_modify.php');
		}
		else
		{
		_close();
...
{
	//获取数据
	$_rows =_fetch_array("SELECT username,sex,phone,email FROM tb_user WHERE username= '{$_COOKIE['username']}'");
	if($_rows)
	{
		$_html= array();
