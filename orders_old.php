...
//订单删除操作
if($_GET['action'] == 'delete' && isset($_GET['order_id'])){
	$_order_id = $_GET['order_id'];
	_query("DELETE FROM tb_order_items WHERE order_id = $_order_id");
	_query("DELETE FROM tb_orders WHERE id = $_order_id");
	if(_affected_rows() == 1){
		_close();
		...
		<tr><th>商品信息</th><th>商品名称</th><th>单价</th><th>操作</th></tr>
			...
			<tr class="orders">
				<td colspan="4">
					...
					<span ><?php echo '已完成';?></span>
					<span class="delete">[<a href="orders.php?action=delete&order_id=<?php echo $_html['order_id']?>">删除</a>]	</span>
				</td>			
			</tr>
			<?php 
				$_result2 = _query("SELECT 
										...
										WHERE 
											ord.order_id='{$_html['order_id']}' AND of.id=ord.food_id
										");
			...
			<tr class="product">			
				...
				<td>[<a href="comment.php?id=<?php echo $_html['food_id']?>&order_id=<?php echo $_html['order_id']?>">评价</a>]</td>
			</tr>
